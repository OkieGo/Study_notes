...
|   Handler：处理器（控制器）   |  在DispatcherServlet的控制下Handler对具体的用户请求进行处理  |
| HandlerAdapter：处理器适配器  |       通过HandlerAdapter对处理器（控制器方法）进行执行       |
|   ViewResolver：视图解析器    | 进行视图解析，得到相应视图。例如ThymeleafView、InternalResourceView、RedirectView |
|          View：视图           |                 将模型数据通过页面展示给用户                 |

## DispatcherServlet初始化过程

**在DispatcherServlet初始化时 init() -->  initServletBean()  -->  initWebApplicationContext()  --> {  createWebApplicationContext()  -->  ( wac.setParent(parent)  return wac )-->  DispatcherServlet.onRefresh()初始化9大组件  -->  this.getServletContext().setAttribute(attrName, wac)设置到ServletContext域中  }**
...
service()  -->  processRequest()  -->  doService()  -->  doDispatch()  -->{ preHandler，handlerAdapter.handle(),  postHandler,  processDispatcherResult()视图渲染,  afterCompletion }
